; ============================
; Pure SIR on Watts–Strogatz (robust)
; ============================
extensions [nw]
...
turtles-own [
  infected?     ;; boolean: true if infectious (I)
  resistant?    ;; boolean: true if recovered/immune (R)
]
...
to setup
  clear-all
  setup-nodes
  setup-watts-strogatz-network
  ;; force all nodes to S (blue) just before seeding
  ask turtles [ become-susceptible ]
  ;; seed infections (cap to population just in case)
  if initial-outbreak-size > 0 [
    ask n-of (min list initial-outbreak-size count turtles) turtles [ become-infected ]
  ]
  ask edges [ set color white ]

  ;; --- COERCE any numeric leftovers to booleans (defensive) ---
  ask turtles [
    if is-number? infected?  [ set infected?  (infected?  != 0) ]
    if is-number? resistant? [ set resistant? (resistant? != 0) ]
  ]

  ;; network structure metrics (guarded)
  set mean-path-length -1
  carefully [ set mean-path-length nw:mean-path-length ] [ set mean-path-length -1 ]
  set avg-clustering   -1
  carefully [ set avg-clustering   mean [ nw:clustering-coefficient ] of turtles ] [ set avg-clustering -1 ]

  ;; init dynamics trackers
  set t_peak 0
  set I_max  count turtles with [ infected? = true ]
  set R_final 0
...
;; IMPORTANT: average-node-degree (k) must be EVEN and at least 2
to setup-watts-strogatz-network
  if (average-node-degree mod 2) != 0 [
    user-message "average-node-degree must be even for a WS network. Please set an even value."
    stop
  ]
  if average-node-degree < 2 [
    user-message "average-node-degree must be at least 2 for WS. Increase k."
    stop
  ]
  ;; Build WS: N nodes, mean degree k, rewiring p
  nw:generate-watts-strogatz turtles edges
    number-of-nodes average-node-degree rewire-probability
...
; ============================
; GO (Pure SIR, per-neighbor)
; ============================

to go
  if count turtles with [ infected? = true ] = 0 [
    set R_final (count turtles with [ resistant? = true ]) / number-of-nodes
    stop
  ]

  ;; 1) Stage per-neighbor infections
  let newly-infected (turtle-set nobody)
  ask turtles with [ infected? = true ] [
    ask link-neighbors with [ resistant? = false and infected? = false ] [
      if random-float 100 < virus-spread-chance [
        set newly-infected (turtle-set newly-infected self)
      ]
    ]
  ]

  ;; 2) Stage recoveries (pure SIR: I -> R)
  let to-recover turtles with [ infected? = true and (random-float 100 < recovery-chance) ]

  ;; 3) Apply updates simultaneously (avoid order bias)
  ask newly-infected [ become-infected ]
  ask to-recover     [ become-resistant ]

  ;; 4) Track peak time & value
  let I count turtles with [ infected? = true ]
  if I > I_max [
    set I_max I
    set t_peak ticks
  ]
...
to become-infected   ;; turtle proc
  set infected?  true
  set resistant? false
  set color red
end

to become-susceptible ;; turtle proc
  set infected?  false
  set resistant? false
  set color blue
end

to become-resistant  ;; turtle proc (absorbing R)
  set infected?  false
  set resistant? true
  set color gray
  ask my-edges [ set color gray - 2 ]
end

; ============================
; REPORTERS for plotting (optional)
; ============================

to-report S-fraction
  report (count turtles with [ infected? = false and resistant? = false ]) / number-of-nodes
end

to-report I-fraction
  report (count turtles with [ infected? = true ]) / number-of-nodes
end

to-report R-fraction
  report (count turtles with [ resistant? = true ]) / number-of-nodes
end
@#$#@#$#@
GRAPHICS-WINDOW
265
